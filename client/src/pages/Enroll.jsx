            ...
            <tbody>
              {enrolledCourses.map(course => (
                <tr key={course._id} style={{ borderBottom: '1px solid #eee' }}>
                  ...
                  <td>{course.time}</td>
                  <td>
                    <button onClick={() => navigate(`/notices?course=${encodeURIComponent(course.name)}`)} style={{ background: '#b39ddb', color: '#fff', border: 'none', borderRadius: 4, padding: '6px 12px', fontWeight: 500, cursor: 'pointer', marginRight: 8 }}>공지사항</button>
            ...
            <tbody>
              {periods.map((period, i) => (
                <tr key={period}>
                  <td>{period}교시</td>
                  {days.map((day, j) => (
                    <td key={day} style={{ minWidth: 100, whiteSpace: 'pre-line', background: timetable[i][j] ? courseColorMap[timetable[i][j]?.split('\n')[0]] || '#e3f2fd' : undefined, color: '#222', fontWeight: timetable[i][j] ? 600 : undefined }}>
                      {timetable[i][j] || ''}
                    </td>
                  ))}
                </tr>
